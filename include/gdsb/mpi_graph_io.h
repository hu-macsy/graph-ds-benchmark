...
inline BinaryGraphHeaderMetaDataV3 read_binary_graph_header(MPI_File const input)
{
    ...
    case 2:
    {
        ...
        BinaryGraphHeaderMetaDataV3 meta_data;
        error = MPI_File_read_all(input, &meta_data, sizeof(BinaryGraphHeaderMetaDataV3), MPI_CHAR, &status);
        if (error != MPI_SUCCESS)
        {
...
{
    BinaryGraphHeaderMetaDataV3 const data = read_binary_graph_header(input);
...
std::tuple<Vertex64, uint64_t> read_binary_graph_partition(MPI_File const input,
                                                           BinaryGraphHeaderMetaDataV3 const& data,
                                                           ReadF&& read,
